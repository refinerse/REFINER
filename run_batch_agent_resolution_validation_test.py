#!/usr/bin/env python3
"""Batch validation-test guided agent resolution with Qwen Code in Docker."""
...
def write_summary(output_dir: Path, all_results: list[dict], elapsed: float) -> dict:
    """Write summary.json with aggregate validation-guided repair and assessment stats."""
    total_comments = sum(r.get("num_comments", 0) for r in all_results)
    # num_resolved is groundtruth-based; keep consistent with per-instance result.json.
    ...
    total_expected_failures = sum(
        r.get("num_expected_failures_observed", 0) for r in all_results
    )
    total_initial_unexpected_passes = sum(
        r.get("num_initial_unexpected_passes", 0) for r in all_results
    )
    total_validation_tests_revised = sum(
        r.get("num_validation_tests_revised", 0) for r in all_results
    )
    total_gt_tests = sum(
        (r.get("groundtruth_assessment") or {}).get("num_tests", 0) for r in all_results
    )
    total_gt_passed = sum(
        (r.get("groundtruth_assessment") or {}).get("num_passed", 0) for r in all_results
    )
    ...
    for result in all_results:
        ...
        row = repo_data[repo]
        gt = result.get("groundtruth_assessment") or {}
        row["instances"] += 1
        row["comments"] += result.get("num_comments", 0)
        row["resolved"] += gt.get("num_passed", 0)   # groundtruth-based
        row["expected_failures_observed"] += result.get("num_expected_failures_observed", 0)
        row["initial_unexpected_passes"] += result.get("num_initial_unexpected_passes", 0)
        row["validation_tests_revised"] += result.get("num_validation_tests_revised", 0)
        row["gt_tests"] += gt.get("num_tests", 0)
        row["gt_passed"] += gt.get("num_passed", 0)

    for row in repo_data.values():
        row["resolution_rate"] = row["gt_passed"] / row["gt_tests"] if row["gt_tests"] else None
        ...
        "instance_results": [
            {
                ...
                "num_expected_failures_observed": result.get("num_expected_failures_observed", 0),
                "num_initial_unexpected_passes": result.get("num_initial_unexpected_passes", 0),
                "num_validation_tests_revised": result.get("num_validation_tests_revised", 0),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch validation-test guided agent resolution using Qwen Code"
    )
    parser.add_argument(
        "--dataset-file",
        type=str,
